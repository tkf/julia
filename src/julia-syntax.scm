      ...
      (if (pair? stmts)
          (let ((e (car stmts)))
            (cond ((and (pair? e) (eq? (car e) 'line))
                   ...
                   (set! linetable (cons `(line ,current-line ,current-file) linetable))
...
(define (renumber-lambda lam)
  ...
  (let* ((code (build-linetable (lam:body lam)))
         (locs (cadr code))
         (linetab (caddr code))
         (code (car code)))
